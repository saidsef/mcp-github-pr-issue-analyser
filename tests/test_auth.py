...
class TestBuildRedisClient:
    ...
    def _kwargs(self, uri, redis_password=None):
        with patch("mcp_github.auth.AsyncRedis") as mock_cls, \
             patch("mcp_github.auth.REDIS_PASSWORD", redis_password):
...
class TestBuildTokenStore:
    ...
    def test_redis_store_constructed_with_correct_client(self):
        ...
        with patch("mcp_github.auth.REDIS_HOST_PORT", "redis://localhost:6379"), \
